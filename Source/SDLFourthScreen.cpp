...
void Level2::init() {
	...
	{
		window = SDL_CreateWindow(" Level2 ", 0, 0, 1366, 768,SDL_WINDOW_MAXIMIZED);
...
void Level2::loadSurface()
{
	S[0] = IMG_Load("Level2.jpg");
...
void Level2::loadRect()
{
	...
	SrcBackSurface[0].w = 640;
	SrcBackSurface[0].h = 815;
...
void Level2::Gameloop()
{
	...
	while (!running)
	{
		...
		if (ZoombieCount > 6)
		{
			...
			if (ZoombieCount < 7) 
			{
				std::uniform_int_distribution<> velZoombie(10,20);
			...
			if (SDL_IntersectRect(&DestFontSurface[1], &DestFontSurface[4], &DestFontSurface[5]) == SDL_TRUE)
			{
				...
				if (count < 4)
				{
					std::uniform_int_distribution<> distZommbie(windowx - 10, windowx + 10);
					DestFontSurface[4].x = distZommbie(gen);
...
bool Level2::loadmusic()
{
	chunk[0] = Mix_LoadWAV("Windows Navigation Start.wav");
	music[0] = Mix_LoadMUS("musLevel2.mp3");
	return true;
}
